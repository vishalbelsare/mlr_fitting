...
def mlr_level_matvec_base(l1, B_l1, C_l1, l2, B_l2, C_l2, row_lk):
    # Compute A_l1 @ \tilde A_l2
    assert l1 <= l2
    B = B_l1 + 0
    if row_lk[l2].size-1 == C_l1.shape[0]:
        # \tilde A_l2 is diagonal
        d = np.multiply(B_l2, C_l2).sum(axis=1).reshape(-1, 1)
        C = d * C_l1
    else:
        #  (r_C_l1 x num_blocks_l2 * r_B_l2)
        C_l1t_B_l2 = mult_blockdiag_refined_CtB(C_l1, row_lk[l1], 
                                                B_l2, row_lk[l2])
        indices_Ct2B = np.searchsorted(row_lk[l2], row_lk[l1], side='left') * B_l2.shape[1]
        C = mult_blockdiag_refined_BCt(C_l1t_B_l2, indices_Ct2B, 
                                    C_l2, row_lk[l2])
    return B, C


def mlr_level_matvec(l1, B_l1, C_l1, l2, B_l2, C_l2, row_lk):
    # Compute A_l1 @ \tilde A_l2^T 
    if l1 <= l2:
        return mlr_level_matvec_base(l1, B_l1, C_l1, l2, B_l2, C_l2, row_lk)
    else:
        C, B =  mlr_level_matvec_base(l2, C_l2, B_l2, l1, C_l1, B_l1, row_lk)
        return B, C
...
def mlr_mlr_sum(B1, C1, ranks1, B2, C2, ranks2, row_lk):
    ...
    if B.shape[0] == row_lk[-1].size - 1 and ranks[-1] >= 2:
